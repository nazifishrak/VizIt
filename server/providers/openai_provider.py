    ...
    def generate_voice_over_script(self, code: str) -> None:
        response = self.client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"Create a script for person to do a voice over for the video that gets generated by this code: {code}. The script should not contain any information except what a person should be actually reading. Return the text that will be read by the narator keeping in mind the sync with the video manim will generate its priority should be so that the audio and the video sync. If there is a long equation you dont need to read the whole thing letter for letter Do not use words: Sections, Scene, or anything that is not related to the content of the video. Make sure that when the narator will talk it will match exactly what is presented on the screen which is given by the code above",
                },
            ],
            temperature=0.7,
            max_tokens=15000,
